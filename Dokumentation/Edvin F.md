...
2024-02-06
-----------
Yesterday, I noticed that the part of the frontend that uses the API used outdated code. More specifically, it used an outdated version of the API. The frontend still expected the API to listen on 'http://localhost:5501', which is obsolete. I changed it to use the Wi-Fi IP address of the device on which the API runs, a while back. This means that when I cloned this repository to test the full project, it did not work. With time running out rather rapidly, I decided to revert the IP-related changes entirely. So, as a result, the API only works on the same device on which it runs, once more. It's a little sad, but I think the previous approach had some security flaws anyway. This'll be a point of improvement for anyone potentially continuing work on this project, I suppose. At least it would be, if I was able to merge the pull request as should be possible. Apparently some Netlify tests or something are failing, and I've no idea why. I *think* I can merge the pull request anyway, but I don't want to do so without asking the others first. Also, as a sidenote, I tried to dockerize the backend real quick since I changed from the IP-approach to localhost again. Alas, although I was able to fully run the container this time, this error message fell upon me: `Unable to bind to http://localhost:5501 on the IPv6 loopback interface: 'Cannot assign requested address'`. So it kind of worked, but not quite. I'll, again, have to leave it here and work on other things.

2024-02-07
-----------
When looking through 'Server_C#/API_Documentation', I noticed that some parts of it were outdated. I updated those things and rephrased some other parts to make the documentation more clear and cohesive. The outdated text stated that the API was still listening on the Wi-Fi IP address of the device on which it runs. I reverted this yesterday (at least I think it was yesterday?) due to compatibility and whatnot, but, admittedly, all the changing back and forth has me a tad confused. I may have made similar misses somewhere else, but I hope it's all up to date.
